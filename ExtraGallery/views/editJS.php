...
$( document ).ready(function() {
    ...
    var $maincontent = $('#maincontent'),
        ...
        // $insertRow = $imagesTable.find('tr.insert-row').remove(), //row used to generate insert rows
		...
		insertIndex = 0, //image row index to insert 
		insertMethod = 'Before',
        lastValidationName = '',
		...
		deleteLocked = <?php echo json_encode(in_array($galleryData["name"] ?? '', $settings["delete-lock"] ?? '')); ?>;
    ...
    $('body').find('button.add.eg-button').click(function(event){
		insertIndex = Math.max(0, $imagesTable.find('> tbody > tr.image-row').length); // do not allow to negative
		insertMethod = 'Before';
	...
	//add row row
    $imagesTable.on('click', '.eg-button.insert', function(){
		insertIndex = $(this).closest('.image-row').index();
		insertMethod = 'Before';
    ...
    $positionPopup.on('click', '.eg-button', function(event){
        ...
        if (!isNaN(newPos) && newPos > 0 && newPos <= len){
            ...
            var newIdx = newPos == 1 ? 0 : newPos - 1;
            if (currPos > newPos){
                $row.insertBefore($imagesTable.find('.image-row:eq('+newIdx+')'));
            }
            else{
                $row.insertAfter($imagesTable.find('.image-row:eq('+newIdx+')'));
            }
	...
	$editGallery.submit(function(event){
		...
		//make it lowercase, used to compare old and new
		if ($nameInput.val())        
			$nameInput.val($nameInput.val().toLowerCase());
        ...
        //user changed gallery name so create field that stores old gallery name to delete 
	...
	function onImageSelected(filename, width, height, leftCalls){
        var $row = createRow(filename, width, height);
			insertJqueryMethod = 'insert' + insertMethod; 
		
        $row[insertJqueryMethod]($imagesTable.find('> tbody > tr:eq('+ insertIndex +')'));
    ...
    function createRow(filename, width, height, itemData){
		...
		// var $newInsertRow = $insertRow.clone();
		...
		return $new;
        // return $newInsertRow.add($new);
    }
    
    function renumberRows(){
        $imagesTable.find('.image-row span.number').each(function(index){
           $(this).text(index + 1);
        });
    }

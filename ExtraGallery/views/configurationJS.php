$( document ).ready(function() {
    ...
    $configForm.submit(function(event){
        ...
        var error = false,
			...
			$deleteLockInput = $configForm.find('textarea[name="delete-lock"]');
		...
		if ( $deleteLockInput.val() && !$deleteLockInput.val().match(/^[0-9a-z-]+(,[0-9a-z-]+)*$/gi) ){
            error = true;
            $deleteLockInput.addClass('not-valid');
        }
        else{
            $deleteLockInput.removeClass('not-valid');
        }   
    ...
    function fillThumbnailsData(){
        $.each( thumbnailsData, function( index, thumb ) {
            var $thumbSection = $emptyThumnailSettings.clone().insertBefore('.section.security');

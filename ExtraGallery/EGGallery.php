...
{    
	...
	*/
    public static function validateGalleryName($instanceNum, $name){
		$name = strtolower($name); //always lower case
	...
	*/
            ...
            foreach ($xml->items->item as $itemXml){
                ...
                for ($t = 0; $t < count($settings['thumbnails']); $t++) {
                    ...
                    if ( $settings['thumbnails'][$t]['enabled'] ){
                        $thumb = [];
                        $thumb['filename'] = (string)$itemXml->{'thumb-'.$t}->filename ?: null;
                        $thumb['width'] = (int)$itemXml->{'thumb-'.$t}->width ?: null;
                        $thumb['height'] = (int)$itemXml->{'thumb-'.$t}->height ?: null;
                        $item['thumb-'.$t] = $thumb;
                    }
                }   
    ...
    public static function save($instanceNum, $data){
        $settings = EGSettings::load($instanceNum);
	...
	*/
    public static function parse($settings){
        ...
        for ($i = 0; isset($_POST[$i.'-filename']); $i++) {
            ...
            for ($t = 0; $t < count($settings['thumbnails']); $t++) {
                if ( $settings['thumbnails'][$t]['enabled'] )
                    $row['thumb-' . $t] = ['filename' => $_POST[$i.'-thumb-'. $t], 'width' => null, 'height' => null];
            }   
            
            for ($f = 0; $f < count($settings['fields']); $f++) {
                for ($l = 0; $l < $langCount; $l++) {  //create language versions
                    $row['languages'][$l]['field-' . $f] =  @$_POST[$i.'-'.$f.'-'.$l]; //silent error, checkbox will be not sended when not checked

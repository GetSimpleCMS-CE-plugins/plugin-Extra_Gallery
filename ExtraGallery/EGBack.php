...
class EGBack{
    ...
    public function init(){
        ...
        if ($this->currentFile == 'load.php' && @$_GET['id'] == $this->pluginId){
            add_action('header', [$this, 'onHeader'] ); 
            add_action('footer', [$this, 'onFooter'] ); 
        }
        ...
        add_action( 'plugins-sidebar', 'createSideMenu', 
			[$this->pluginId, i18n_r(EG_ID.'/CONF_SIDEBAR'). ($this->instanceNum ? (' - '.$this->instanceNum) : '' ), 'conf'] 
		); 
		
        add_action( 'nav-tab', 'createNavTab', [$this->pluginTitleFull, $this->pluginId, $this->settings['tab-label'], 'list'] );
		
		add_action($this->pluginTitleFull.'-sidebar', 'createSideMenu', [$this->pluginId, i18n_r(EG_ID.'/LIST_SIDEBAR_LIST'), 'list']);
		
		if(!$this->settings['create-lock'])
			add_action($this->pluginTitleFull.'-sidebar', 'createSideMenu', [$this->pluginId, i18n_r(EG_ID.'/LIST_SIDEBAR_NEW'), 'add']);
		
		add_action($this->pluginTitleFull.'-sidebar', 'createSideMenu', [$this->pluginId, i18n_r(EG_ID.'/LIST_SIDEBAR_EDIT'), 'edit', false]);
		
		// var_dump($this->settings['create-lock']);
    }
    ...
    private function _routeAction (){
        ...
        if (isset($_GET['edit']) && isset($_GET['save'])){   
            ...
            $data = EGGallery::parse($this->settings);
            $toDelete = @$_POST['delete'];
					
			//some security checks
			$editedExists = !EGGallery::validateGalleryName($this->instanceNum, $data['name']);
			
			//create lock, gallery not exists, and nothing for delete here so we not changing name
			// ... && ... && to delete passed ? gallery for deletion exists = you can change galleries names but never add new gall
			if ($this->settings['create-lock'] && !$editedExists && !($toDelete ? !EGGallery::validateGalleryName($this->instanceNum, $toDelete) : false))
				die('create lock');
				
			//are you trying to delete gallery that is locked?
			//if you trying to rename locked gallery 
			if($toDelete ? in_array($toDelete, $this->settings['delete-lock']) : false)
				die('delete lock');

            $message = EGGallery::save($this->instanceNum, $data);
            $isError = $message ? 1 : '';
            ...
            if (!$isError && $toDelete)
                EGGallery::delete($this->instanceNum, $toDelete);
                 
            EGTools::cleanUnusedThumbs();

            redirect('load.php?id='.$this->pluginId.'&edit&name='.strtolower($_POST['name']).'&message='.urlencode($message).'&isError='.$isError);
        }

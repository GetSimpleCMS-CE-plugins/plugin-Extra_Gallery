...
class EGSettings {
    ...
    private static $_settingsStorage = [];
    
    private static $_allowedTypes = ['text', 'textarea', 'wysiwyg', 'checkbox', 'select'];
	...
	*/
    public static function load($instanceNum, $createDefaults = false){
        ...
        $data['required-width-ranges'] = [];
        for ($i=0; $i < count($xml->{'required-width-ranges'}->range); $i++) {
            $data['required-width-ranges'][] = [0 => (int)$xml->{'required-width-ranges'}->range[$i]->from, 1 => (int)$xml->{'required-width-ranges'}->range[$i]->to];
        } 
		...
		$data['required-height-ranges'] = [];
        for ($i=0; $i < count($xml->{'required-height-ranges'}->range); $i++) {
            $data['required-height-ranges'][] = [0 => (int)$xml->{'required-height-ranges'}->range[$i]->from, 1 => (int)$xml->{'required-height-ranges'}->range[$i]->to];
        } 
		
		
		//security section
		$data['create-lock'] = (string)$xml->{'create-lock'};
        $data['delete-lock'] = [];
        for ($i=0; $i < count($xml->{'delete-lock'}->gallery); $i++) {
            $data['delete-lock'][] = (string)$xml->{'delete-lock'}->gallery[$i];
        } 
        ...
        for ($i=0; $i < count($xml->fields->field); $i++) {
            ...
            if ($data['fields'][$i]['type'] == 'select')
                $data['fields'][$i]['options'] = (string)$f->options ? explode(',',(string)$f->options) : []; //empty after explode will give one value, prevent it
    ...
    public static function save($instanceNum){
		...
		//security section
		$xml->addChild('create-lock', $data['create-lock']);
        $deleteLock = $xml->addChild('delete-lock');
        for ($i = 0; $i < count($data['delete-lock']); $i++) {
            $deleteLock->addChild('gallery')->addCData($data['delete-lock'][$i]);
        }
    ...
    private static function _parse()
    {
        ...
        $data['languages'] = $_POST['languages'] ? explode(',',$_POST['languages']) : []; 
		...
		$data['required-width-ranges'] = $_POST['required-width-ranges'] ?  array_map('self::_explodeRangeRow', explode(';', $_POST['required-width-ranges'])) : []; //twice explode
		...
		$data['required-height-ranges'] = $_POST['required-height-ranges'] ? array_map('self::_explodeRangeRow', explode(';', $_POST['required-height-ranges'])) : []; //twice explode
		
		//SECURRITY section
		$data['create-lock'] = (bool) @$_POST['create-lock'];
		$data['delete-lock'] = $_POST['delete-lock'] ? explode(',',$_POST['delete-lock']) : []; 
    ...
    private static function _validate($data){
        ...
        for ($i = 0; $i < count($data['thumbnails']); $i++) {
            ...
            if ($thumb['enabled']){
				...
				if (!in_array($thumb['auto-crop'], ['', 'fill']))
					return false;
            }
        }
        return true;

...
while ($file = readdir($dir_handle)) {
	if ($file == "." || $file == ".." || $file == ".htaccess" ){
		...
		$dirFiles = is_array($dirFiles) ? $dirFiles : [];
		
		$dirsArray[$folder] = ['path' => $subPath.$folder, 'num' => 0];
		...
		if ($ext == 'jpg' || $ext == 'jpeg' || $ext == 'gif' || $ext == 'png') {
			...
			$filesArray[] = ['filename' => $file, 'filepath' => $subPath.$file, 'size' => fSize($ss['size']), 'width' => $width, 'height' => $height];
		}
	}
}

...
class EGImage
{
	 ...
	 * @var array
	 */
	protected $_errors = ['NotExists'         => 'The file %s does not exist', 'NotReadable'       => 'The file %s is not readable', 'Format'            => 'Unknown image format: %s', 'GD'                => 'The PHP extension GD is not enabled', 'WidthHeight'       => 'Please specify at least one of the width and height parameters', 'CropDimExceed'     => 'The cropping dimensions must be smaller and within original ones', 'InvalidResource'   => 'Invalid image resource provided', 'CannotSave'   		=> 'Cannot save image file: %s'];
	 ...
	 * @var array
	 */
	protected $_acceptedFormats = ['png', 'gif', 'jpeg'];
	...
	public function resize($width = null, $height = null, $mode = 'fit', $sharpen = false)
	{
		...
		if($sharpen && function_exists('imageconvolution')) {
			$intSharpness = $this->_findSharp($this->_originalSize[0], $width);
				$arrMatrix = [[-1, -2, -1], [-2, $intSharpness + 12, -2], [-1, -2, -1]];
